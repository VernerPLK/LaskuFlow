...
export default async function InvoiceDetailPage(props: { params: Promise<{ invoiceId: string }> }) {
  ...
  if (!invoice) {
    return <main className="pageShell"><div className="card"><h1>Laskua ei löytynyt</h1></div></main>;
  }
    ...
    <main className="pageShell">
      <section className="card heroCard">
        <div>
          ...
          <p className="muted">{invoice.customers?.name} · status {invoice.status}</p>
        </div>
        ...
        <div className="card">
          ...
          {paymentUrl ? (
            <div className="stack">
              ...
              <a className="button primary" href={paymentUrl}>Avaa julkinen maksusivu</a>
              <a className="button" href={`/api/invoices/${invoice.id}/pdf`}>Lataa PDF-lasku</a>
            </div>
          ) : <p className="muted">Tälle laskulle ei ole vielä maksulinkkiä.</p>}
        </div>

        <div className="card">
          <h2>Onnistuneet maksut</h2>
          {(payments || []).length === 0 ? <p className="muted">Ei maksuja vielä.</p> : payments?.map((payment: any) => (
            <div className="lineRow" key={payment.id}>
              <span>{payment.provider_key || payment.payment_method}</span>
              <span>{payment.reconciliation_status}</span>
              <strong>{eur(Number(payment.amount))}</strong>
              <span>{new Date(payment.created_at).toLocaleString('fi-FI')}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="card">
          ...
          {(sessions || []).map((session: any) => (
            <div className="lineRow" key={session.id}>
              <span>{session.provider_key}</span><span>{session.status}</span><strong>{eur(Number(session.amount))}</strong><span>{session.selected_method}</span>
            </div>
          ))}
        </div>
        <div className="card">
          ...
          {(events || []).map((event: any) => (
            <div className="lineRow" key={event.id}>
              <span>{event.event_type}</span><span>{event.status}</span><strong>{event.amount ? eur(Number(event.amount)) : '-'}</strong><span>{event.provider_key}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid two" style={{ marginTop: 20 }}>
        <div className="card">
          <h2>Webhook-tapahtumat</h2>
          {(webhooks || []).map((delivery: any) => (
            <div className="lineRow" key={delivery.id}>
              <span>{delivery.provider_key}</span><span>{delivery.status}</span><strong>{delivery.signature_valid ? 'Allekirjoitus ok' : 'Ei vahvistettu'}</strong><span>{delivery.event_type || '-'}</span>
            </div>
          ))}
        </div>
        <div className="card">
          ...
          {(logs || []).map((log: any) => (
            <div className="lineRow" key={log.id}>
              <span>{statusText(log.action)}</span><span>{new Date(log.created_at).toLocaleString('fi-FI')}</span><strong>{log.entity_type}</strong><span></span>
            </div>
          ))}
        </div>
      </section>
